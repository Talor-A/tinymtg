...
function resolveBeanstalk(
	...
	const card = engine.spawnCard(state, "up-the-beanstalk", controller, "hand");
...
describe("Up the Beanstalk", () => {
	...
	test("draws a card when you cast a spell with mana value 5", () => {
		...
		// "5 or greater".
		const thor = engine.spawnCard(state, "thor-odinson", ALICE, "hand");
	...
	test("does not trigger on a spell with mana value 4", () => {
		...
		const whisperer = engine.spawnCard(state, "beast-whisperer", ALICE, "hand");
		fillManaPool(state, ALICE);
	...
	test("does not trigger from hand", () => {
		const state = setupMain(engine);
		engine.spawnCard(state, "forest", ALICE, "library");
		engine.spawnCard(state, "up-the-beanstalk", ALICE, "hand");
